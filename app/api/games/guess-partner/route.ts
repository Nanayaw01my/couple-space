...
export async function GET() {
  try {
    ...
    await connectDB();
    const answers = await GuessPartner.find({ coupleId }).sort({ questionId: 1 });
    return NextResponse.json(answers);
  } catch (err: any) {
    console.error('[guess-partner GET]', err);
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const doc = await GuessPartner.findOneAndUpdate(
      { coupleId, questionId },
    ...
    console.error('[guess-partner PATCH]', err);
    return NextResponse.json({ error: err?.message || 'Server error' }, { status: 500 });
  }
}
...
export async function DELETE() {
  try {
    ...
    await connectDB();
    await GuessPartner.updateMany({ coupleId }, { $set: { answers: [] } });
    return NextResponse.json({ ok: true });
  } catch (err: any) {
    console.error('[guess-partner DELETE]', err);
    return NextResponse.json({ error: err?.message || 'Server error' }, { status: 500 });
  }
}
