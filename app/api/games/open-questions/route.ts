...
import OpenQuestion from '@/lib/models/OpenQuestion';
import { OPEN_QUESTIONS } from '@/lib/questions';
...
export async function GET() {
  ...
  await connectDB();
  const current = await OpenQuestion.findOne({ coupleId, status: 'answering' }).sort({ roundNumber: -1 });
  const history = await OpenQuestion.find({ coupleId, status: 'done' }).sort({ roundNumber: -1 }).limit(20);
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.action === 'start') {
      const existing = await OpenQuestion.findOne({ coupleId, status: 'answering' });
      if (existing) return NextResponse.json({ error: 'Round already in progress' }, { status: 400 });
      const count = await OpenQuestion.countDocuments({ coupleId });
      const promptIndex = count % OPEN_QUESTIONS.length;
      const round = await OpenQuestion.create({
        coupleId, roundNumber: count + 1, promptIndex,
        question: OPEN_QUESTIONS[promptIndex], answers: [],
      });
      return NextResponse.json(round);
    }

    if (body.action === 'answer') {
      const round = await OpenQuestion.findOne({ coupleId, status: 'answering' });
      ...
      round.answers.push({ userId, userName: session.user.name, text });
    ...
    console.error('[open-questions POST]', err);
...
export async function DELETE() {
  ...
  await connectDB();
  await OpenQuestion.deleteMany({ coupleId });
